...
func (c *Controller) apply(ctx context.Context, target model.Target, mode model.ExposureMode, selectedProviderKey string, selectedRouteMode model.ExposureMode, confirmFunnel, confirmExternal bool, timeout time.Duration, approval *MutationApproval) (receipt model.OperationReceipt, applyErr error) {
	tx := mutationTransaction{controller: c, target: target, mode: mode, selectedProviderKey: selectedProviderKey, selectedRouteMode: selectedRouteMode, confirmFunnel: confirmFunnel, confirmExternal: confirmExternal, timeout: timeout, approval: approval}
	return tx.run(ctx)
}

func (tx mutationTransaction) run(ctx context.Context) (receipt model.OperationReceipt, applyErr error) {
	c := tx.controller
	target := tx.target
	mode := tx.mode
	selectedProviderKey := tx.selectedProviderKey
	selectedRouteMode := tx.selectedRouteMode
	confirmFunnel := tx.confirmFunnel
	confirmExternal := tx.confirmExternal
	timeout := tx.timeout
	approval := tx.approval
	if err := target.Validate(); err != nil {
		return model.OperationReceipt{}, model.WrapError(model.ErrInvalidInput, "exposure", err.Error(), false, "invalid", "Select a valid target and retry.", err)
	}
	target = target.Normalized()
	if mode != model.ExposureServe && mode != model.ExposureFunnel && mode != model.ExposureDisabled {
		return model.OperationReceipt{}, model.NewError(model.ErrInvalidInput, "exposure", "unsupported exposure mode", false, "invalid", "Choose disabled, serve, or funnel.")
	}
	if mode == model.ExposureFunnel && !confirmFunnel {
		return model.OperationReceipt{}, model.NewError(model.ErrUnsafe, "exposure", "Funnel requires explicit public-internet confirmation", false, "not_confirmed", "Confirm the exact target with `--confirm-public` or the TUI confirmation screen.")
	}
	if c.Provider == nil || c.Discoverer == nil {
		return model.OperationReceipt{}, model.NewError(model.ErrDependency, "exposure", "exposure providers are unavailable", true, "unavailable", "Run the readiness check and retry.")
	}
	c.mu.Lock()
	if c.busy == nil {
	...
	operationKey := target.Key()
	operationStarted := c.now()
	operationID := model.StableID("apply", operationKey, string(mode), operationStarted.UTC().Format(time.RFC3339Nano))
	receipt = model.OperationReceipt{ID: operationID, StartedAt: operationStarted}
	c.busy[operationKey] = true
	c.desired[operationKey] = mode
	c.operationStates[operationKey] = model.ExposureApplying
	c.operationStarted[operationID] = operationStarted
	c.recordEventLocked(model.OperationEvent{OperationID: operationID, Phase: "start", At: operationStarted, Target: target, Mode: mode, Capability: operationCapability(mode), State: model.ExposureApplying})
	c.mu.Unlock()
	defer func() {
		if receipt.ID == "" {
		...
		c.recordEventLocked(model.OperationEvent{OperationID: operationID, Phase: "final", At: receipt.FinishedAt, Duration: eventDuration(started, receipt.FinishedAt), Target: target, Mode: mode, Capability: operationCapability(mode), State: state, ErrorCode: operationErrorCode(applyErr)})
	...
	c.mu.Lock()
	for i := range exposures.Routes {
		route := &exposures.Routes[i]
	...
	if mode == model.ExposureDisabled {
		if len(routeIDs) == 0 {
			return model.OperationReceipt{ID: model.StableID("noop-disable", target.Key()), StartedAt: c.now(), FinishedAt: c.now(), Verified: true}, nil
		}
		if len(routeIDs) != 1 {
	...
	if len(routeIDs) == 1 {
		...
		if !replaceExisting {
			receipt := model.OperationReceipt{ID: operationID, StartedAt: operationStarted, FinishedAt: c.now(), Verified: true}
...
func (c *Controller) rollbackReplacement(_ context.Context, requested model.Target, previous model.ExposureRoute) error {
...
func (c *Controller) requireCurrentListener(ctx context.Context, target model.Target) error {
...
func (c *Controller) requireModeReady(ctx context.Context, mode model.ExposureMode) error {
...
func (c *Controller) verifyMode(ctx context.Context, target model.Target, mode model.ExposureMode, expectedProviderKey, expectedService, expectedPath, expectedBackend string) (model.ExposureRoute, error) {
...
func (c *Controller) verifyAbsent(ctx context.Context, target model.Target, routeID string) (bool, model.ExposureSnapshot, error) {
	deadline := time.NewTimer(2 * time.Second)
	defer deadline.Stop()
	for {
		snapshot, err := c.Provider.List(ctx)

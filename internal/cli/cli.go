...
type response struct {
	...
	Errors        []fault.SafeError `json:"errors,omitempty"`
}

type exposureStatusData struct {
	View      exposure.View            `json:"view"`
	Readiness readinessmodel.Readiness `json:"readiness"`
}

// Run executes one CLI invocation. The command package owns parsing and
// reporting; bootstrap owns construction of the concrete adapters.
func Run(args []string, stdin *os.File, stdout, stderr io.Writer, dependencies bootstrap.App) int {
	a := app{
		config:            dependencies.Config,
		discoverer:        dependencies.Discoverer,
		processTerminator: dependencies.ProcessTerminator,
		tailscale:         dependencies.Tailscale,
	}
...
func (a app) run(args []string, stdin *os.File, stdout, stderr io.Writer) int {
	if len(args) == 0 {
		if stdin == nil || !isTTY(stdin) || !isTTYFile(stdout) {
			printError(stderr, fault.NewError(fault.ErrInvalidInput, "cli", "tailge requires a TTY for the interactive interface", false, "non_interactive", "Use `tailge scan`, `tailge exposure status`, or another CLI command."))
			return 2
		}
		return tui.Run(stdin, stdout, stderr, a.discoverer, a.processTerminator, a.tailscale, a.config)
	}
	...
	default:
		printError(stderr, fault.NewError(fault.ErrInvalidInput, "cli", "unknown command "+strconv.Quote(args[0]), false, "invalid", "Run `tailge --help` for available commands."))
		return 2
	}
}
...
func (a app) scan(args []string, stdout, stderr io.Writer) int {
	...
	if a.discoverer == nil {
		return reportMaybeJSON(stdout, stderr, fault.NewError(fault.ErrDependency, "discovery", "listener discovery is unavailable", true, "unavailable", "Retry with a supported platform adapter."), parsed.bools["json"])
	}
	...
	if scanErr != nil {
		...
		return fault.AsAppError(scanErr).Exit
	}
	return 0
}
...
func (a app) exposure(args []string, stdout, stderr io.Writer) int {
	...
	selectedRouteMode := exposuredata.ExposureDisabled
	if parsed.values["mode"] != "" {
		...
		selectedRouteMode = exposuredata.ExposureMode(parsed.values["mode"])
		if selectedRouteMode != exposuredata.ExposureServe && selectedRouteMode != exposuredata.ExposureFunnel {
	...
	var applyErr error
	if subcommand == "disable" && selectedRouteMode != exposuredata.ExposureDisabled {
...
func (a app) exposureStatus(args []string, stdout, stderr io.Writer) int {
	...
	readiness := readinessmodel.Readiness{At: time.Now(), Status: readinessmodel.ReadinessUnknown}
	var readinessErr error
	if a.tailscale == nil {
		readinessErr = fault.NewError(fault.ErrDependency, "cli", "Tailscale provider is unavailable", true, "unavailable", "Configure Tailscale and retry.")
	} else {
	...
	if refreshErr != nil && len(view.Items) == 0 {
		...
		return fault.AsAppError(refreshErr).Exit
	}
	...
	if readinessErr != nil {
		...
		return fault.AsAppError(readinessErr).Exit
	}
	if cfgErr != nil {
		printError(stderr, cfgErr)
		return fault.AsAppError(cfgErr).Exit
	}
	return 0
}
...
func (a app) configCommand(args []string, stdout, stderr io.Writer) int {
	...
	case "show":
		...
		if err != nil {
			...
			return fault.AsAppError(err).Exit
		}
...
func (a app) doctor(args []string, stdout, stderr io.Writer) int {
	...
	defer cancel()
	if a.tailscale == nil {
		err := fault.NewError(fault.ErrDependency, "doctor", "Tailscale provider is unavailable", true, "unavailable", "Configure Tailscale and retry.")
		return reportDoctor(stdout, stderr, readinessmodel.Readiness{At: time.Now(), Status: readinessmodel.ReadinessUnknown}, err, parsed.bools["json"])
	}
	...
	if parsed.values["probe"] != "" {
		if readiness.Daemon != readinessmodel.ReadinessReady || readiness.Identity != readinessmodel.ReadinessReady || readiness.Connected != readinessmodel.ReadinessReady {
			err := fault.NewError(fault.ErrDependency, "doctor", "compatibility probe requires a ready Tailscale node", false, "not_ready", "Fix the daemon, identity, and connection checks before probing route mutations.")
			return reportDoctor(stdout, stderr, readiness, err, parsed.bools["json"])
		}
		if !parsed.bools["confirm-test-route"] {
			return reportDoctor(stdout, stderr, readiness, fault.NewError(fault.ErrUnsafe, "doctor", "compatibility probe requires --confirm-test-route", false, "not_confirmed", "Supply a disposable listener target and explicit confirmation."), parsed.bools["json"])
		}
		var (
		...
		if parsed.bools["self-test-listener"] {
			...
			if err != nil {
				return reportDoctor(stdout, stderr, readiness, fault.WrapError(fault.ErrDependency, "doctor", "could not open a disposable loopback listener", true, "unavailable", "Choose an available local TCP interface and retry.", err), parsed.bools["json"])
			}
			...
			var parseErr error
			target, parseErr = targetmodel.ParseTarget(parsed.values["target"], "tcp")
			if parseErr != nil {
				return reportDoctor(stdout, stderr, readiness, fault.WrapError(fault.ErrInvalidInput, "doctor", parseErr.Error(), false, "invalid", "Provide a disposable address:port.", parseErr), parsed.bools["json"])
			}
		}
		...
		probeErr := probeRunner.Run(ctx, &cfg, target, exposuredata.ExposureMode(parsed.values["probe"]), parsed.bools["confirm-public"])
		if probeErr != nil {
...
func openSelfTestListener() (net.Listener, targetmodel.Target, error) {
	listener, err := net.Listen("tcp4", "127.0.0.1:0")
	if err != nil {
		return nil, targetmodel.Target{}, err
	}
	tcpAddress, ok := listener.Addr().(*net.TCPAddr)
	if !ok || tcpAddress.Port < 1 || tcpAddress.Port > 65535 {
		_ = listener.Close()
		return nil, targetmodel.Target{}, errors.New("self-test listener returned an invalid TCP address")
	}
	return listener, targetmodel.Target{Address: "127.0.0.1", Port: tcpAddress.Port, Protocol: "tcp"}, nil
}

func reportDoctor(stdout, stderr io.Writer, readiness readinessmodel.Readiness, err error, jsonOutput bool) int {
	return reportDoctorWithCode(stdout, stderr, readiness, err, jsonOutput, errorCode(err, readinessExit(readiness)))
}
func reportDoctorWithCode(stdout, stderr io.Writer, readiness readinessmodel.Readiness, err error, jsonOutput bool, code int) int {
	if err == nil && code != 0 {
...
func readinessStatusError(r readinessmodel.Readiness, code int) error {
	if code == fault.ErrVerification.ExitCode() || r.Status == readinessmodel.ReadinessUnknown {
		return fault.NewError(fault.ErrVerification, "doctor", "Tailscale readiness is unknown", true, "unknown", "Retry `tailge doctor --tailscale` after checking Tailscale status.")
	}
	return fault.NewError(fault.ErrDependency, "doctor", "Tailscale readiness is "+string(r.Status), true, string(r.Status), "Complete the reported readiness checks before enabling exposure.")
}

func readinessExit(r readinessmodel.Readiness) int {
	if len(r.Modes) == 0 {
		return fault.ErrVerification.ExitCode()
	}
	for _, mode := range r.Modes {
		...
		if mode.Status == readinessmodel.ReadinessNotReady {
			return fault.ErrDependency.ExitCode()
		}
		if mode.Status != readinessmodel.ReadinessReady {
			return fault.ErrDependency.ExitCode()
		}
	}
	if r.Status == readinessmodel.ReadinessUnknown {
		return fault.ErrVerification.ExitCode()
	}
	if r.Status != readinessmodel.ReadinessReady {
		return fault.ErrDependency.ExitCode()
	}
	return 0
}
...
func errorsFrom(values ...any) []fault.SafeError {
	result := []fault.SafeError{}
	seen := map[string]bool{}
	for _, value := range values {
		var safe fault.SafeError
		switch value := value.(type) {
		case nil:
			continue
		case error:
			appErr := fault.AsAppError(value)
			if appErr == nil {
				continue
			}
			safe = appErr.Safe()
		case *fault.SafeError:
			if value == nil {
				continue
			}
			safe = *value
...
func reportError(w io.Writer, err error) int { printError(w, err); return fault.AsAppError(err).Exit }
...
func invalidArgs(message string) *fault.AppError {
	return fault.NewError(fault.ErrInvalidInput, "cli", message, false, "invalid", "Run `tailge --help` for command syntax.")
}

func writeJSON(stdout, stderr io.Writer, value response) int {
	data, err := json.MarshalIndent(value, "", "  ")
	if err != nil {
		printError(stderr, fault.WrapError(fault.ErrOperation, "cli", "cannot encode JSON response", false, "failed", "Retry the command.", err))
		return 6
	}
...
func parseTarget(raw, address, protocol string) (targetmodel.Target, error) {
	if strings.IndexFunc(address, func(r rune) bool { return r < 0x20 || r == 0x7f || r == '\\' || r == '"' }) >= 0 {
		return targetmodel.Target{}, fmt.Errorf("address contains unsupported control or quoting characters")
	}
	if address != "" && !strings.Contains(raw, ":") {
		raw = net.JoinHostPort(targetmodel.NormalizeAddress(address), raw)
	}
	return targetmodel.ParseTarget(raw, protocol)
}
...
func isTTYFile(writer io.Writer) bool {
...
func printReadiness(w io.Writer, r readinessmodel.Readiness) {
